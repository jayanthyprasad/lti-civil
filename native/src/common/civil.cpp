  ...
  (JNIEnv *pEnv, jobject jObj)
{
	...
	jmethodID jNativeCaptureDeviceInfoSetNoOutputs = pEnv->GetMethodID(jNativeCaptureDeviceInfoClass, "setNoOutputs", "(I)V");
	jmethodID jNativeCaptureDeviceInfoSetOutputName = pEnv->GetMethodID(jNativeCaptureDeviceInfoClass, "setOutputName", "(ILjava/lang/String;)V");
	jmethodID jNativeCaptureDeviceInfoSetNoInputs = pEnv->GetMethodID(jNativeCaptureDeviceInfoClass, "setNoInputs", "(II)V");
	jmethodID jNativeCaptureDeviceInfoSetInputName = pEnv->GetMethodID(jNativeCaptureDeviceInfoClass, "setInputName", "(IILjava/lang/String;)V");
	...
	for(i=L.begin(); i != L.end(); ++i)
	{
		...
		pEnv->CallVoidMethod(jNativeCaptureDeviceInfo, jNativeCaptureDeviceInfoSetNoOutputs, noOutputs);
		for (int j = 0; j < noOutputs; j++) {
			const wchar_t *outputName = (*i).getOutputName(j);
			jchar *jOutputName = wchar_t_to_jchar_array_alloc(outputName);
			jstring jOutputNameString = pEnv->NewString(jOutputName, wcslen(outputName));
			pEnv->CallVoidMethod(jNativeCaptureDeviceInfo, jNativeCaptureDeviceInfoSetOutputName, j, jOutputNameString);
			delete[] jOutputName;
			int noInputs = (*i).getNoInputs(j);
			pEnv->CallVoidMethod(jNativeCaptureDeviceInfo, jNativeCaptureDeviceInfoSetNoInputs, j, noInputs);
			for (int k = 0; k < noInputs; k++) {
			    const wchar_t *inputName = (*i).getInputName(j, k);
			    jchar *jInputName = wchar_t_to_jchar_array_alloc(inputName);
			    jstring jInputNameString = pEnv->NewString(jInputName, wcslen(inputName));
				pEnv->CallVoidMethod(jNativeCaptureDeviceInfo, jNativeCaptureDeviceInfoSetInputName, j, k, jInputNameString);
				delete[] jInputName;
			}
		}
...
JNIEXPORT jobject JNICALL Java_com_lti_civil_impl_jni_NativeCaptureSystem_openCaptureDeviceStreamOutput
  (JNIEnv *pEnv, jobject jObj, jstring jDeviceIDStr, jint output, jint input)
{
	...
	try
	{
		pCaptureStreamPeer = pPeer->openCaptureDeviceStream(szDeviceIDStr, output, input);
...
{
	return Java_com_lti_civil_impl_jni_NativeCaptureSystem_openCaptureDeviceStreamOutput(pEnv, jObj, jDeviceIDStr, 0, 0);
}
  ...
  (JNIEnv *pEnv, jobject jObj)
{
	...
	jmethodID jNativeVideoFormatConstructor = pEnv->GetMethodID(jNativeVideoFormatClass, "<init>", "(JIIIFI)V");
  ...
  (JNIEnv *pEnv, jobject jObj, jobject jVideoFormat)
{
	...
	jmethodID jNativeVideoFormatGetDataTypeMethodID = pEnv->GetMethodID(jNativeVideoFormatClass, "getDataType", "()I");
    ...
    int dataType = pEnv->CallIntMethod(jVideoFormat, jNativeVideoFormatGetDataTypeMethodID);
	VideoFormat videoFormat(handle, formatType, width, height, fps);
	videoFormat.setDataType(dataType);
...
JNIEXPORT void JNICALL Java_com_lti_civil_impl_jni_NativeCaptureStream_setObserver
  (JNIEnv *pEnv, jobject jObj, jobject jObserver, jint numBuffersHint)
{
	CaptureStream *pCaptureStreamPeer = (CaptureStream *) getPeerPtr(pEnv, jObj);


	if (jObserver != 0)
		pCaptureStreamPeer->setObserver(new JNICaptureObserver(pEnv, pEnv->NewGlobalRef(jObj), pEnv->NewGlobalRef(jObserver), numBuffersHint));	// TODO: dispose properly, obj & refs
	else
